...
public class TechForm extends javax.swing.JFrame
{
    ...
    */
	...
	public TechForm()
	{
        ...
        for (int i=0;i<MAX_TECHS;i++) cTechRec[i] = new TechRec();
		...
		for (int k=0;k<tData.columns.length;k++) {
		    DefaultTableCellRenderer renderer = new
		        DefaultTableCellRenderer();
            renderer.setHorizontalAlignment(
                tData.columns[k].alignment);
            renderer.setFont(
                tData.columns[k].font);
            TableColumn column = new TableColumn(k,
                tData.columns[k].width,renderer,null);
	...
	void TechForm_keyPressed(java.awt.event.KeyEvent event)
	{
		...
		switch(key) {
            ...
            case event.VK_ESCAPE:
                ...
                break;
            case event.VK_F1:
                if (currMode==Lab.IDLE) queryActions();
                break;
            case event.VK_F9:
                if (currMode==Lab.IDLE) this.dispose();
                break;
            case event.VK_HOME:
                if (currMode==Lab.IDLE) {
                    currNdx=0;
	...
	public void finalActions() {
	    ...
	    if (currMode==Lab.ADD) {
	        for (int i=0;i<MAX_TECHS;i++) {
	            if (cTechRec[i].cytotech_code.compareTo((String)ctCode.getText())==0) {
                    String msg="Cytotech "+(String)ctCode.getText()+" already exists";
            ...
            fillForm(currNdx);
            displayRow(currNdx);
            if (!found) format.createErrMsg("Cytotech "+queryCode+" does not exist");
		}
	...
	void ctStatus_keyPressed(java.awt.event.KeyEvent event)
	{
		if (event.getKeyCode()==event.VK_ENTER) {
		    if (format.required(ctStatus,"Active Status")) {
		        if (!ctStatus.getText().equals("A")&&!ctStatus.getText().equals("I"))
		            format.createErrMsg("Status is (A)ctive or (I)nactive");
		        else ctStatus.transferFocus();
		    }
		}
	}
	...
	public void displayTable()
	{
        if (tData.getRowCount()==0) {
            for (int ndx=0; ndx<MAX_TECHS; ndx++) {
                tData.addRow(cTechRec[ndx].cytotech_code,cTechRec[ndx].active_status,
                    cTechRec[ndx].lname,cTechRec[ndx].fname,cTechRec[ndx].mi);
            }
        }
	}
