...
public class ReceiveDateForm extends javax.swing.JFrame
{
    ...
    */
    ...
    private void getDates()
    {
        try {
            String SQL =
                "SELECT TO_CHAR(start_date,'MM/DD/YYYY'), start_lab_number, \n"+
            ...
            while (rs.next() && ndx<8) {
                ...
                switch (ndx) {
                    ...
                    case 1: rDay01.setText(rs.getString(1));
                            ...
                            break;
                    case 2: rDay02.setText(rs.getString(1));
                            ...
                            break;
                    case 3: rDay03.setText(rs.getString(1));
                            ...
                            break;
                    case 4: rDay04.setText(rs.getString(1));
                            ...
                            break;
                    case 5: rDay05.setText(rs.getString(1));
                            ...
                            break;
                    case 6: rDay06.setText(rs.getString(1));
                            ...
                            break;
                    case 7: rDay07.setText(rs.getString(1));
                ...
                e_lab=b_lab-1;
                if (!b_date.equals("0101")) ndx++;
            }
            currMode=Lab.CURRENT;
            try { rs.close(); stmt.close(); }
            catch (SQLException e) { currMode=Lab.IDLE; }
        }
        catch (SQLException e) { currMode=Lab.IDLE; }
    }
    ...
    private void getDates(String qDate)
    {
        try {
            String SQL =
                ...
                "WHERE start_date<=TO_DATE('"+qDate+"','MMDDYYYY')+1 \n"+
            ...
            currMode=Lab.QUERY;
            try { rs.close(); stmt.close(); }
            catch (SQLException e) { currMode=Lab.IDLE; System.out.println(e); }
        }
        catch (SQLException e) { currMode=Lab.IDLE; System.out.println(e); }
    }
		...
		if (event.getKeyCode()==event.VK_ENTER) {
		    if (Utils.required(rDay00,"Receive Date")) {
		        if (Utils.dateVerify(rDay00)) {
		            ...
		            String s2 = s1.substring(4)+s1.substring(0,2)+s1.substring(2,4);
		            int d = (int)Integer.parseInt(s2);
		            if (d>=iDate) rDay00.transferFocus();
		            else {
		                rDay00.setText(sDate);
		                Utils.createErrMsg("New date cannot be earlier than "+sDate);
		            }
		        }
		    }
		}
	...
	void rEnd00_keyPressed(java.awt.event.KeyEvent event)
	{
		if (event.getKeyCode()==event.VK_ENTER) {
		    if (Utils.required(rEnd00,"Ending Lab Number")) {
		        int b = (int)Integer.parseInt(rBegin00.getText());
		        int e = (int)Integer.parseInt(rEnd00.getText());
		        if (e-b>0) { 
	...
	void finalActions()
	{
	    if (currMode==Lab.QUERY) {
	        ...
	        try { 
	            int newLab = (int)Integer.parseInt(rBegin01.getText());
	...
	void ReceiveDateForm_keyPressed(java.awt.event.KeyEvent event)
	{
		...
		switch (key) {
		    ...
		    case event.VK_F3:
		        ...
		        try { 
		            currMode=Lab.UPDATE;
		            saveLab=(int)Integer.parseInt(rBegin01.getText());
		    ...
		    case event.VK_F4:
		        ...
		        int rv = confirmDelete.showConfirmDialog(this,"Delete current receive date",
		            "Delete Rec'v Date",confirmDelete.YES_NO_OPTION,confirmDelete.QUESTION_MESSAGE);
		        if (rv==confirmDelete.YES_OPTION) {
		            int maxLab = 0;
		            try {
		                maxLab = (int)Integer.parseInt(rBegin00.getText());
		            }
		            catch (Exception e) { maxLab=0; }
		            if (maxLab>0) {
	...
	void updateReceiveDates()
	{
	    ...
	    try {
	        int endingLab = ((int)Integer.parseInt(rEnd00.getText()));
	        String SQL = 
	            "UPDATE pcs.receive_dates \n"+
	        ...
	        pstmt.setInt(3,(int)Integer.parseInt(rBegin00.getText()));
	        pstmt.executeUpdate();
	        int inc = 1;
	        int nextStartingLab = ((int)Integer.parseInt(rEnd00.getText()))+1;
	        /* If the current day is a Friday, then the next day to enter
	           a receive date will be on a Monday, or (3) days from the 
	           current date (hence inc=3)
	        */
	        if (d00.getText().equals("FRI")) inc=3;
	        /* Check for special situation:  whether there will
	           be a cross-over from one year to the next
	        */
            ...
            SQL =
	            "SELECT TO_CHAR(MAX(start_date)+?,'MMDD'), \n"+
	        ...
	        for (int i=1; i<=inc; i++) {
	            ...
	            if (isNewYear) {
	                ...
	                // If NYD falls on a Saturday, then next business day is 01/03
	        ...
	        if (isNewYear) {
	            /* Insert an entry into the receive_dates table that has
	               NYD as the date, and completes the range from the
	               highest lab to the zero lab number of the next year;
	               this is considered a "dummy" entry in the table.
	            */
	            SQL =
	                "INSERT INTO pcs.receive_dates \n"+
                ...
                /* Insert the first entry in the receive_dates table for the
                   new year with the appropriate date, and the first lab
                   number of the year (i.e. YYYY000001).
                */
	            SQL =
	                "INSERT INTO pcs.receive_dates (start_date,start_lab_number) \n"+
	            ...
	            SQL =
	                "INSERT INTO pcs.receive_dates (start_date,start_lab_number) \n"+
	                "   SELECT MAX(start_date)+"+inc+",? FROM pcs.receive_dates \n";
	            pstmt=dbConnection.process().prepareStatement(SQL);
	        ...
	        System.out.println("dayAfterNewYears: "+dayAfterNewYears+" nextYear: "+nextYear+"  "+e); 
	        setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        }
        setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
	}
	...
	void deleteReceiveDate(int maxLab)
	{
	    try {
	        String SQL = 
	            "DELETE FROM pcs.receive_dates WHERE start_lab_number = ? \n";
	        PreparedStatement pstmt = dbConnection.process().prepareStatement(SQL);
	        pstmt.setInt(1,maxLab);
	        pstmt.executeUpdate();
	        SQL =
	            "UPDATE pcs.lab_requisitions SET receive_date = NULL \n"+
	            "WHERE lab_number >= ? \n";
	        pstmt = dbConnection.process().prepareStatement(SQL);
	        pstmt.setInt(1,maxLab);
	        pstmt.executeUpdate();
