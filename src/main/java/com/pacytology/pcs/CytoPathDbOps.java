...
public class CytoPathDbOps implements Runnable {
	...
	/*
	 * date results were entered as an integer value in the format YYYYMMDD
	 * yields a strict chronological value that is easily used for date
	 * comparisons
	 */
	int iDatestamp;
	...
	/*
	 * Constructor that sets the CytoPathReport object as the parent of the
	 * CytoPathDbOps (CytoPathology Database Operations) object. Logging for
	 * this object is written to the parent's LogFile object.
	 */
	...
	public synchronized void run() {
		boolean rv = true;
		/*
		 * Determine which query operation to use based on the print mode. For
		 * DRAFT and FINAL mode (i.e. non-queued requests for reports) use
		 * straight query that indicates the lab number range requested.
		 */
		...
		/*
		 * ... otherwise use the queryQueue method which retrieves data for
		 * labReport objects based on which lab numbers are currently in the
		 * print queue for either CURR_DRAFT or CURR_FINAL modes.
		 */
		else {
		...
		if (rv) {
			...
			if (eReports.size() > 0) {
				Export eFile = new Export(Lab.CYTOPATHOLOGY_REPORTS,
						parent.amendedCodes);
	...
	/*
	 * Retrieve data for lab numbers in the specified range: sLab (starting) -
	 * eLab (ending); this as opposed to retrieving the data currently in the
	 * print queue
	 */
	public boolean query(int sLab, int eLab) {
		...
		try {
			...
			while (rs.next()) {
				parent.reportDate = rs.getString(1);
			}
			...
			while (rs.next()) {
				...
				/*
				 * determine which result set takes precedence and retrieve
				 * those results
				 */
				...
				/*
				 * this is a hard coded value to facilitate use of appropriate
				 * date reported on cytology reports; prior to this
				 * implementation the date reported was the date completed of
				 * the screening tech, or QC tech , or pathologist; whichever
				 * codes took precedence. From the time of this implementation
				 * forward the date reported on the cytopathology reports will
				 * be the date the results were input into the system.
				 * LabReportRec.datestamp holds the numeric value of the current
				 * date; in the format YYYYMMDD it is numerically true.
				 */
				...
				if (!Utils.isNull(labReport.test_sent)) {
					if (labReport.test_sent.equals("R")
		...
		if (parent.printMode == Lab.FINAL && parent.labReportVect.size() == 1
				&& (e_report.equals("Y") || e_report.equals("B"))) {
			JOptionPane confirmERept = new javax.swing.JOptionPane();
			int rv = confirmERept.showConfirmDialog(null,
					"Create Electronic Report?", "Electronic Report",
					confirmERept.YES_NO_OPTION, confirmERept.QUESTION_MESSAGE);
	...
	public void setDirector(LabReportRec labReport) {
		...
		try {
			...
			while (rs.next()) {
				labReport.director_name = rs.getString(1);
			}
			try {
				rs.close();
				pstmt.close();
	...
	/*
	 * Query the lab details from the requisition for the lab number indicated
	 * in labReport
	 */
		...
		try {
			...
			while (rs.next()) {
				...
				if (dCodeRec.detail_type.equals("SOURCE"))
					labReport.numSources++;
				else if (dCodeRec.detail_type.equals("DEVICE"))
					labReport.numDevices++;
				else if (dCodeRec.detail_type.equals("CONDITION"))
					labReport.numConditions++;
				else if (dCodeRec.detail_type.equals("OTHER"))
					labReport.numOthers++;
				else if (dCodeRec.detail_type.equals("HISTORY"))
					labReport.numHistory++;
				labReport.numDetails++;
				labReport.detailVect.addElement(dCodeRec);
				dCodeRec = new DetailCodeRec();
	...
	/*
	 * Retrieve the data for all labs CURRENTLY in print queue for the print
	 * mode that is selected (either DRAFTS or FINALS). As results are entered,
	 * the lab is queued for printing; pathologist cases are defaulted for draft
	 * prints first
	 */
	public boolean queryQueue() {
		...
		try {
			...
			/*
			 * If the print mode is for FINAL reports the sort order puts biopsy
			 * request labs first as these ones are tagged to be faxed.
			 */
			if (parent.printMode == Lab.CURR_FINAL)
				...
				SQL += "ORDER BY pr.practice, lab.lab_number \n";
			/*
			 * If the print mode is for DRAFT reports the sort order is by
			 * straight lab number.
			 */
			else
				SQL += "ORDER BY lab.lab_number \n";
			...
			/*
			 * Retrieve the data; the date for each lab number is stored in a
			 * LabReportRec class; each individual lab report object is then
			 * stored in a vector of report objects.
			 */
			while (rs.next()) {
				LabReportRec labReport = new LabReportRec();
				labReport.lab_number = rs.getInt(1);
				...
				queryDetails(labReport);
				/*
				 * determine which result set takes precedence and then retrieve
				 * those result codes
				 */
				setDirector(labReport);
				if (labReport.path_status.equals("Y"))
				...
				ndx++;
				/*
				 * the report counter decrements and is used for display
				 * purposes; once the user clicks the retrieve button they may
				 * visually see the display change starting from the total
				 * number of reports down to zero, at which point the Windows
				 * print windows appears
				 */
				reportCounter--;
				if (parent.printMode == Lab.CURR_DRAFT)
					...
					if (!Utils.isNull(labReport.test_sent)) {
						if (labReport.test_sent.equals("P")
								|| labReport.test_sent.equals("R"))
							labReport.HPVmessage = true;
					}
				} else if (parent.printMode == Lab.CURR_HPV) {
			...
			parent.holdFinalsNdx = parent.labReportVect.size() - numHolds
					- numFaxes;
			parent.log.write("   close labReportVect cursor ["
					+ parent.labReportVect.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
			...
			/*
			 * Begin retrieving the various HPV data; initially set
			 * hasExceptions to false. hasExceptions refers to several special
			 * cases for HPV data. If any one of these cases occurs the boolean
			 * hasExceptions is set to true.
			 */
			parent.hasExceptions = false;
			/*
			 * The HPVrequests vector holds Strings that are formatted lines of
			 * HPV data. This data is added to the Oracle pcs.hpv_requests as
			 * results are entered; the system determines through rules whether
			 * a request for HPV testing indicated on a requisition will
			 * actually be carried out, and if so sets the test_sent field to a
			 * value of R. The HPVrequests vector holds all the R values which
			 ...
			 * by all labs in which the FINAL report should be held until HPV
			 * results are received, and those that do not have to be held.
			 */
			...
			rs = stmt.executeQuery(SQL);
			// flag for message used to separate labs in which FINALs should be
			// held
			...
			while (rs.next()) {
				...
				if (Utils.isNull(b_code) && hold_final.equals("Y")
						&& path_status.equals("Y"))
					s = s.replace('*', ' ');
				if (hold_final.equals("Y")) {
					if (!holdFinals) {
						holdFinals = true;
						/*
						 * Two additional lines that are added to the
						 * HPVrequests vector that are actually not lines of HPV
						 * data; these are the lines on the HPV requisition
						 * sheet that separate the labs in which the practice
						 * requests FINALs be held until HPV results are in,
						 * from those that do not have to be held.
						 */
						parent.HPVrequests.addElement("  ");
						parent.HPVrequests
								.addElement("<-----HOLD FINAL REPORT PENDING HPV TEST RESULTS----->");
					}
				}
				parent.HPVrequests.addElement(s);
			}
			parent.log.write("   close HPVrequests cursor ["
					+ parent.HPVrequests.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
				parent.log.write("queryQueue[3] " + e);
				exitStatus = false;
			}

			/*
			 * Originally an HPV request on the requisition was indicated by
			 * either a Y (yes) or N (no); this was changed to the values of 19
			 * (yes), 20 (yes), and N (no); the 19 and 20 indicate two different
			 * types of HPV testing. For the transitional phase there existed
			 * values of Y (old) and, 19 and 20 (new) for HPV testing requested;
			 * this section of the HPV data sheets listed all lab numbers in
			 * which the decision whether the test should be sent had to be
			 * checked manually (as opposed to the change to having the system
			 * make this determination through use of implemented rules.
			 */
			...
			parent.log.write("   close HPVmanual cursor ["
					+ parent.HPVmanual.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
				parent.log.write("queryQueue[7] " + e);
				exitStatus = false;
			}

			/*
			 * HPV requests in which whenever the system rules determine that
			 * the HPV test should be sent, the physicians office must be called
			...
			parent.log.write("   close HPVpermissions cursor ["
					+ parent.HPVpermissions.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
				parent.log.write("queryQueue[6] " + e);
				exitStatus = false;
			}

			/*
			 * HPV testing is not to be performed by the defaulted lab
			 * contracted to do the work; as of this writing all HPV testing is
			 * sent to CLEARPATH laboratory; however for some practices the HPV
			 * testing is done by an alternate lab instead; this lab is LAB
			 * CORP.
			 */
			...
			// DISCONTINUED
			// There are no tests sent to LAB CORP; code block commented out
			// rather
			// than deleted though.
			/*
			 * SQL =
			 ...
			 * "order by L.lab_number \n";
			 * 
			 * stmt = dbConnection.process().createStatement(); rs =
			 * stmt.executeQuery(SQL);
			 * parent.log.write("   open HPValternates cursor"); while
			 * (rs.next()) { String s = rs.getString(1); String b_code =
			 * rs.getString(2); parent.HPValternates.addElement(s);
			 * parent.hasExceptions=true; } parent.log.write(
			 * "   close HPValternates cursor ["
			 * +parent.HPValternates.size()+"]"); try { rs.close();
			 * stmt.close(); } catch (SQLException e) {
			 * parent.log.write("queryQueue[5] "+e); exitStatus=false; }
			 */

			/*
			 * HPV tests in which the doctor must be verified before the test is
			 * sent out.
			 */
			...
			parent.log.write("   close HPVdrVerify cursor ["
					+ parent.HPVdrVerify.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
				parent.log.write("queryQueue[8] " + e);
				exitStatus = false;
			}
			...
			parent.log.write("   close HPVonly cursor ["
					+ parent.HPVonly.size() + "]");
			try {
				rs.close();
				stmt.close();
			} catch (SQLException e) {
				parent.log.write("queryQueue[9] " + e);
				exitStatus = false;
			}
		} catch (Exception e) {
			parent.log.write(e);
	...
	/*
	 * Retrive the result set for this labReport object. These results are those
	 * in which the results indictated by the original screening tech take
	 * precedence.
	 */
	public boolean queryResults(LabReportRec labReport) {
		...
		try {
			...
			while (rs.next()) {
				...
				resultCodeRec.description = getCodeDescription(
						resultCodeRec.bethesda_code, s);
				/*
				 * SPECIAL CONDITION: This account does not want to see a 507
				 * code on their reports.
				 */
				if (labReport.practice == 428
						&& resultCodeRec.bethesda_code.equals("507"))
					continue;
	 ...
	 * labReport object is one that was a Quality Control case, hence the result
	 * set indicated by the QC screening tech take precedence.
	 */
	public boolean queryQCResults(LabReportRec labReport) {
		...
		try {
			...
			while (rs.next()) {
				...
				resultCodeRec.active_status = "Y";
				/*
				 * old data; unknown tech code was 999; replace with with QCT
				 * (Quality Control Tech) on report.
				 */
	...
	/*
	 * Retrive the result set for this labReport object. This particular
	 * labReport object is one that required screening by a pathologist, hence
	 * the pathologist result code set takes precedence.
	 */
	public boolean queryPathResults(LabReportRec labReport) {
		...
		try {
			...
			labReport.numResults = labReport.resultVect.size();
			/*
			 * On a pathologist case, if the labReport object was also a Quality
			 * Control case, then the QC tech's initials must also appear on the
			 * report.
			 */
	 ...
	 * CytoPathology Report print queue, and then add a print history record to
	 * the print history table. Each account of a CytoPathology report being
	 * printed, DRAFT or FINAL, must be recorded.
	 */
	public boolean dequeue(int labNum) {
		parent.log.write("dequeue(" + labNum + "," + parent.printMode + ")");
		boolean exitStatus = true;
		try {
			String SQL = new String("DELETE FROM pcs.cytopath_print_queue \n"
			 ...
			 * date the report was printed. The print mode will reveal whether
			 * the lab was printed as a DRAFT or FINAL from the current print
			 * queue, or whether it was printed as a special request to print a
			 * DRAFT or FINAL. SPECIAL NOTE: As of this writing (04/14/2005)
			 * there have been cases in which the print mode recorded was
			 * NO_PRINT and this situation corresponds to cases in which
			 * duplicate reports were printed; the nature of this bug has not
			 * been solved yet.
			 */
			SQL = new String(
					"INSERT INTO pcs.cytopath_history (lab_number,first_print,print_date) \n"
							+ "VALUES (" + labNum + "," + parent.printMode
							+ ",SysDate) \n");
	...
	public void holdReportForHPV(int labNumber) {
		PreparedStatement pstmt = null;
		try {
			String SQL = "UPDATE pcs.cytopath_print_queue \n"
					+ "SET first_print = ? \n" + "WHERE lab_number = ?";
			pstmt = DbConnection.process().prepareStatement(SQL);
	...
	/*
	 * Determine whether there are any labReport objects currently queued for
	 * printing; the return value is the sum of any DRAFT or FINAL reports that
	 * are currently queued for printing.
	 */
	public int checkQueue() {
		try {
			String SQL = new String("SELECT first_print,count(first_print) \n"
			...
			while (rs.next()) {
				...
				if (pMode == Lab.CURR_DRAFT)
					parent.numDrafts = rs.getInt(2);
				else if (pMode == Lab.CURR_FINAL)
					parent.numFinals = rs.getInt(2);
				else if (pMode == Lab.CURR_HPV)
					parent.numHPVs = rs.getInt(2);
			}
			try {
				rs.close();
				stmt.close();
		...
		parent.log.write("RETURNING checkQueue("
				+ (parent.numDrafts + parent.numFinals + parent.numHPVs) + ")");
	 ...
	 * passed in are the result code and the CURRENT description for that code.
	 * The table that is queried contains code descriptions for any codes in
	 * which the description has changed at least once. For these codes the
	 * correct description that was active at the original date the report was
	 * printed must be used on any subsequent copies of the report. This is
	 * determined based on the date the prior description was terminated
	 * compared to the original date the results were entered.
	 */
	private String getCodeDescription(String rCode, String rDescr) {
		...
		try {
			...
			while (rs.next()) {
				...
				/*
				 * If the termination date for the description for this code is
				 * later than the original date of the results, set the return
				 * description to this value and break.
				 */
				if (ds > iDatestamp) {
					uDescr = t;
					break;
			...
			parent.log.write(e + " getCodeDescription");
		}
		/*
		 * After the prior description was queried for rCode, if the value of
		 * the return value uDescr is NULL, then no prior description was
		 * located; hence set the return value to the current description which
		 * was passed in.
		 */
	...
	/*
	 * For all lab numbers in the HPV requests table that were flagged for an
	 * HPV requisition (i.e. value of 'R'), update the value of test_sent to 'P'
	 * (i.e. HPV requisition was printed, test was sent, and HPV results are now
	 * Pending).
	 */
	public boolean HPVupdate() {
		int dCount = DbConnection.getRowCount("PCS.HPV_REQUESTS",
				"TEST_SENT='R'");
		boolean exitStatus = true;
		parent.log.write("HPVupdate()   count==" + dCount);
		try {
			String SQL = new String("UPDATE pcs.hpv_requests SET \n"
			...
			try {
				...
				parent.log.write("HPVupdate[1] " + e);
				exitStatus = false;
			}
		} catch (Exception e) {
	...
	/*
	 * Called at instansiation of CytoPathReport object, returns a Vector of
	 * Strings of all results codes that indicate the CytoPathology report is an
	 * amended report.
	 */
	public Vector getAmendedCodes() {
		...
		try {
			...
			try {
				...
				parent.log.write("getAmendedCodes[1] " + e);
			}
		} catch (Exception e) {
	...
	public Vector getCoverSheets(int prac) {
		...
		try {
			...
			if (prac > 0)
				SQL.append("WHERE practice=" + prac + "\n");
			else
				SQL.append("WHERE cover_sheet='Y' \n");
			...
			try {
				...
				parent.log.write("getCoverSheets[1] " + e);
			}
			if (coverSheets.size() == 0) {
				String errMsg = null;
				if (prac > 0)
					errMsg = "No data located for Account #" + prac;
				else
					errMsg = "There are no cover sheets queued to print";
	...
	public void resetCoverSheets() {
		try {
			...
			try {
				...
				parent.log.write("resetCoverSheets[1] " + e);
			}
		} catch (Exception e) {
