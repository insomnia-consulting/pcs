...
public class FaxLetterDialog extends javax.swing.JDialog {
	...
	private void getLetterCounts() {
		ResultSet rs = null;
		Statement stmt = null;
		String SQL = "SELECT letter_type,COUNT(letter_type) \n"
	...
	private boolean checkPrint() {
		...
		if (genericLtr.isSelected()) {
			canPrint = true;
			if (genericLetterCount > 0)
				fileNames.addElement("generic.ltr");
			else
				fileNames.addElement("generic.old");
		}
		if (diagnosisLtr.isSelected()) {
			canPrint = true;
			if (diagnosisLetterCount > 0)
				fileNames.addElement("diag.ltr");
			else
				fileNames.addElement("diag.old");
		}
		if (nmnLtr.isSelected()) {
			canPrint = true;
			if (nmnLetterCount > 0)
				fileNames.addElement("medicare.ltr");
			else
				fileNames.addElement("medicare.old");
		}
		return (canPrint);
	}

	/*
	 * private void printLetters() { PrintJob pjob; Properties p = new
	 * java.util.Properties(); Graphics pgraphics; String name = new
	 * String("Fax Letters"); pjob=getToolkit().getPrintJob(this,name,p); if
	 * (pjob!=null) {} pjob.end(); }
	 */
