...
begin
      ...
      */
      ...
      select qc_status,path_status into L_qc_status,L_path_status
      from pcs.lab_results where lab_number=L_num;
      ...
      */
      ...
      if (L_path_status='Y') then
         /* lab was screened by a pathologist 
            so initially assume charges 
         */
         screening_level:=PATHOLOGIST;
         /* if there is not an 012 or 010 code then 
            no pathologist charges
         */
         ...
         /* if there was a 13R code and an 040 code then
            no pathologist charges; otherwise there is
         */
         select count(*) into rcnt from pcs.pathologist_control_codes
         ...
         /* same logic is applied again for a QC case */
         ...
         /* same logic is applied again for a regular screening case */
         if (screening_level<>PATHOLOGIST) then
            screening_level:=PATHOLOGIST;
      ...
      /* Determine which CPT code to use based on the type
         of lab preparation; note that for BS billing, if
         the primary diagnosis code begins with the letter 
         'V', then use the Medicare CPT code; otherwise
         use the regular CPT code. Also note that if this
         is a surgical biopsy, then additional info from
         the requisition is needed to determine which
         code to use.
      */
      P_code_area:='PREP TYPE';
      if (L_prep=CONVENTIONAL) then
         if (L_diagnosis_code LIKE 'V%') then
      ...
      /* retrieve the cost based on the procedure code that was determined
         and the pricing used for this labs account
      */
      ...
      /*
         If this is a sugical biopsy, then the item cost is multiplied
         by the number of vials.
      */
      ...
      /* if after all checks for pathologist charges indicate yes
         (i.e. value is one) then add procedure code 88141 and
         retrieve the pricing for this account
      */
      if (screening_level=PATHOLOGIST) then
