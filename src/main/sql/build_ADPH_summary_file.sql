...
begin
   ...
   S_file_name:=cbuf1||RTRIM(LTRIM(TO_CHAR(cycle)))||'.whp';
   ...
   heading3:=margin||'MONTH OF '||S_period||'  ['||LTRIM(RTRIM(TO_CHAR(cycle)))||']';
   ...
   loop
	 ...
	 ADPH_city,ADPH_state,ADPH_zip;
      ...
      where practice=ADPH_account
      and statement_id=S_month;
      if (rcnt<=0) then
	 goto loop_top;
      end if;
      -- EACH INDIVIDUAL ADPH PROGRAM HAS BEEN (UNFORTUNATELY) HARDCODED
      -- 	 RATHER THAN OPENING A CURSOR!
	 ...
	 UTL_FILE.PUTF(file_handle,'%s\n',margin||ADPH_address2); line_num:=line_num+1;
      ...
      -- WOMEN'S HEALTH PROGRAM == FAMILY PLANNING
      ...
      cbuf1:=margin||chr(27)||chr(71)||'	     PATIENTS SEEN UNDER FAMILY PLANNING PROGRAM ['||
	 ...
	 UTL_FILE.PUTF(file_handle,'%s\n',curr_line); line_num:=line_num+1;
      ...
      -- WOMEN'S HEALTH PROGRAM == MATERNITY PROGRAM
      ADPH_whp:='MAT';
      select count(distinct A.lab_number) into rcnt
      from pcs.practice_statement_labs A, pcs.adph_lab_whp B
      ...
      and B.adph_program=ADPH_whp
      and A.billing_cycle=cycle;
      cbuf1:=margin||chr(27)||chr(71)||'		PATIENTS SEEN UNDER MATERNITY PROGRAM ['||
	 LTRIM(RTRIM(TO_CHAR(rcnt)))||']'||chr(27)||chr(72);
	 ...
	 UTL_FILE.PUTF(file_handle,'%s\n',curr_line); line_num:=line_num+1;
      ...
      -- WOMEN'S HEALTH PROGRAM == ALABAMA BREAST AND CERVICAL CANCER
      ADPH_whp:='ABCCEDP';
      select count(distinct A.lab_number) into rcnt
      from pcs.practice_statement_labs A, pcs.adph_lab_whp B
      where A.lab_number=B.lab_number
	 ...
	 UTL_FILE.PUTF(file_handle,'%s\n',curr_line); line_num:=line_num+1;
      ...
      -- WOMEN'S HEALTH PROGRAM == GYN (THIS IS STUPID!! DESCRIPTION AND CODE ARE THE SAME!!)
      ADPH_whp:='GYN';
      select count(distinct A.lab_number) into rcnt
      from pcs.practice_statement_labs A, pcs.adph_lab_whp B
      where A.lab_number=B.lab_number
	 ...
	 UTL_FILE.PUTF(file_handle,'%s\n',curr_line); line_num:=line_num+1;
      ...
      -- WOMEN'S HEALTH PROGRAM == NO PROGRAM IDENTIFICATION
      ...
      and B.adph_program=ADPH_whp
      and A.billing_cycle=cycle;
      cbuf1:=margin||chr(27)||chr(71)||'	     PATIENTS SEEN WITHOUT PROGRAM IDENTIFICATION ['||
	 LTRIM(RTRIM(TO_CHAR(rcnt)))||']'||chr(27)||chr(72);
      UTL_FILE.PUTF(file_handle,'\n%s\n',cbuf1); line_num:=line_num+2;
      UTL_FILE.PUTF(file_handle,'%s\n',dline); line_num:=line_num+1;
      if (rcnt>0) then
