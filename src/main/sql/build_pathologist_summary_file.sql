create or replace procedure     build_pathologist_summary_file
(
   S_month in number
)
as
   ...
   cursor pathologist_list is
      ...
      WHERE lr.datestamp>=begin_date
      AND lr.datestamp<end_date

      AND lr.pathologist=p.pathologist_code
      AND lr.pathologist is NOT NULL;
   S_pathologist number;
   S_pathologist_initials varchar2(4);
   S_lname varchar2(32);
   S_fname varchar2(32);
   ...
   cursor daily_total is
      ...
      WHERE pathologist=S_pathologist_initials
...
begin
   ...
   P_proc_name:='BUILD_PATHOLOGIST_SUMMARY_FILE';
   P_code_area:='PREP';
   ...
   S_file_name:=TO_CHAR(last_date,'MONYYYY')||'.pth';
   ...
   loop
	 ...
	 num_conv, num_tp, num_img
      ...
      AND B.pathologist=S_pathologist_initials
	 ...
	 P_code_area:='P_CLASS: '||S_pathologist_initials||','||TO_CHAR(p_code,'99');
   ...
   -- 	 do not count.
	 ...
	 P_code_area:='ADEQUACY: '||S_pathologist_initials||','||b_code;
	 SELECT
	    NVL(SUM(DECODE(A.preparation,CONVENTIONAL,1,0)),0),
	    NVL(SUM(DECODE(A.preparation,THIN_PREP,1,NON_GYN,1,0)),0),
	    NVL(SUM(DECODE(A.preparation,IMAGED,1,0)),0)
	 INTO
	    num_conv, num_tp, num_img
	 FROM pcs.lab_requisitions A, pcs.lab_results B, pcs.pathologist_control_codes C
	 WHERE A.lab_number=B.lab_number
   ...
   --    Prepare variables for printing
	 ...
	 LPAD('CONV   PREP    IMG  TOTAL',41);
      ...
      SELECT lname,fname INTO S_lname,S_fname
      FROM pcs.pathologists WHERE pathologist=S_pathologist;
      prior_mode:=NEXT_PATH;
	 ...
	 if (prior_mode=NEXT_PATH) then
	    ...
	    UTL_FILE.PUTF(file_handle,'%s\n\n',curr_line);
	    curr_line:=MARGIN||'PATHOLOGIST:  '||LTRIM(S_lname)||', '||LTRIM(S_fname);
	    UTL_FILE.PUTF(file_handle,'\n%s\n',curr_line);
	    curr_line:=MARGIN||'CODE:	     '||S_pathologist_initials;
	    UTL_FILE.PUTF(file_handle,'%s\n\n',curr_line);
	 end if;
	 ...
	 if (curr_mode=SLIDES_SCREENED) then
	    ...
	    curr_line:=curr_line||
	       ...
	       LPAD(TO_CHAR(pth_pcnt,'990.99'),10)||' %'||
...
exception
   ...
   when OTHERS then
      ...
      values (P_error_code,P_error_message,P_proc_name,P_code_area,SysDate,UID,S_pathologist);
      commit;
      RAISE;
